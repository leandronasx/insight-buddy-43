-- ------------------------------------------------------------
-- 1. gerar_lembretes_automacoes_v3 (gera a notificação diária)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.gerar_lembretes_automacoes_v3(
    p_id_empresa UUID,
    p_hoje TEXT
)
RETURNS VOID
    ...
    LOOP
        ...
        leads_calc AS (
            SELECT
                ...
                l.momento_funil,
                (l.data_contato AT TIME ZONE 'America/Sao_Paulo')::DATE AS data_contato,
                (l.data_orcamento AT TIME ZONE 'America/Sao_Paulo')::DATE AS data_orcamento,
                (
                    SELECT (v.data_servico AT TIME ZONE 'America/Sao_Paulo')::DATE
        ...
        agrupados AS (
            ...
            WHERE NOT EXISTS (
                ...
                WHERE ha.id_leads = elegiveis.lead_id
                  ...
                  AND (ha.data_criacao AT TIME ZONE 'America/Sao_Paulo')::DATE = v_hoje_date
            )
            GROUP BY tipo_lembrete
        )

        -- Primeiro DELETA tudo que havia sido gerado hoje para garantir que dados antigos sumam
        DELETE FROM public.lembretes_automacoes
        WHERE id_empresa = v_empresa.id
          AND data_execucao = v_hoje_date;

        -- Depois insere os leads que bateram exatamente com a regra hoje
...
-- ------------------------------------------------------------
-- 2. fn_get_cadencia_leads_v3 (habilita os botões de lead individual)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_get_cadencia_leads_v3(
    p_empresa_id UUID,
    p_lead_ids   UUID[],
    p_hoje       TEXT
)
